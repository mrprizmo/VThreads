...
class Mutex {
  ...
  class MutexAwaiter {
   public:
    ...
    void await_suspend(vthread::VThread* vthread) {
      ...
      while (true) {
        schedulers::task::TaskBase* old_head = mutex_.state_.load(std::memory_order_acquire);

        if (old_head == &mutex_.dummy_) {
          if (mutex_.state_.compare_exchange_strong(old_head, nullptr,
        ...
        if (mutex_.state_.compare_exchange_weak(old_head, waiter,
 ...
 public:
  ...
  void Lock() {
    ...
    if (state_.compare_exchange_strong(unlocked_state, nullptr,
                                      std::memory_order_acquire,
                                      std::memory_order_relaxed)) {
      return;
    }
  ...
  bool TryLock() {
    auto* unlocked_state = &dummy_;
    return state_.compare_exchange_strong(unlocked_state, nullptr,
                                          std::memory_order_acquire,
                                          std::memory_order_relaxed);
  }
  ...
  void Unlock() {
    if (local_list_.load(std::memory_order_acquire)) {
      ...
      local_list_.store(static_cast<schedulers::task::TaskBase*>(new_owner->next_), std::memory_order_release);

      new_owner->prev_ = new_owner->next_ = nullptr;
      schedulers::ScheduleTask(vthread::VThread::Self().CurrentView(), new_owner);
      return;
    }

    auto* waiters_stack = state_.exchange(nullptr, std::memory_order_acq_rel);

    if (waiters_stack == nullptr) {
      schedulers::task::TaskBase* expected = nullptr;

      if (state_.compare_exchange_strong(expected, &dummy_,
                                        std::memory_order_release,
                                        std::memory_order_relaxed)) {
        return;
      }
